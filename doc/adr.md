...
- This is deliberate, not an oversight: `SkillUserInput.path` (`schemas/ClientRequest.json`) is the app-server's own `skills/list` output, not a user-selected file. It identifies an absolute `SKILL.md` file, and `scope: "user"`/`"system"`/`"admin"` paths routinely live outside the workspace. Requiring existence or workspace containment would silently drop catalog entries that the app-server has already resolved.
