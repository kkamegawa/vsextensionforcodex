...
a newer push or survive lifecycle invalidation. After each `TurnCompleted` conversation event, the
view model awaits a forced rate-limit read after completing the existing transcript projection;
this bypasses the TTL while preserving the last successful snapshot when the read fails.
