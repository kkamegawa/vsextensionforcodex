...
| 次ターン設定 | `/fast`, `/model`, `/personality`, `/plan`, `/reasoning` | 次の`turn/start`へ渡す型付きフィールドを更新します。 |
...
`/goal`は表示、設定、編集、一時停止、再開、クリアに対応し、目的は
1～4,000文字です。
...
非対応コマンド、未知コマンドを分離します。未知コマンドは最大3件の候補を返し、
`turn/start`にも`turn/steer`にも送りません。

ターン実行中は`/status`、`/mcp`、goal表示だけ即時実行します。それ以外は
スレッド単位の最大10件FIFOキューへ入れます。同じ設定コマンドが待機中なら、
キュー位置を維持したまま最新値に置換します。ターン完了後に順次実行し、
...
処理し、未加工のJSON payloadをトランスクリプトへ渡しません。
